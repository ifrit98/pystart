...
"""
# TODO: Create conda yaml for setting up env for brian?
...
# List of necessary ML dependencies:
...
package (which may not be provided from older redistributable packages). The redistributable 
comes with Visual Studio 2019 but can be installed separately:
    ...
    3) Download and install the Microsoft Visual C++ Redistributable for Visual Studio 2015, 
        2017 and 2019 for your platform.
...
Verify the install:
    (venv) C:\> python -c "import tensorflow as tf;print(tf.reduce_sum(tf.random.normal([1000, 1000])))"

Use Windows WHL link:
Python 3.9 CPU-only - https://storage.googleapis.com/tensorflow/windows/cpu/tensorflow_cpu-2.8.0-cp39-cp39-win_amd64.whl
...
try:
    import numpy as np
    from numpy import array, asarray, allclose, alltrue
    from numpy import sqrt, sin, cos, log, log10, log2, exp, power
    from numpy import mod, mean, median, var, std, argmax, argmin
...
rmatrix = lambda x: np.random.normal(size=maybe_list(x))
...
np_onehot = lambda values: np.eye(np.max(values) + 1)[values]
